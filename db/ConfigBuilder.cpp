...
#define IP_USER_RULE                                                             \
    ...
    void BuildConfigSingBox(const std::shared_ptr<BuildConfigStatus> &status) {
        ...
        if (dataStore->tor_system_proxy && !status->forTest) {
    ...
    QString WriteVPNSingBoxConfig() {
        QString default_proxy_out = "neko-socks";
        QString tor_outbound = "";
        if (dataStore->tor_tun_mode) {
            default_proxy_out = "tor-out";
            tor_outbound = R"({ "type": "tor", "tag": "tor-out", "detour": "neko-socks" },)";
        }
        
        // tun user rule
        auto match_out = dataStore->vpn_rule_white ? default_proxy_out : "direct";
        auto no_match_out = dataStore->vpn_rule_white ? "direct" : default_proxy_out;
        ...
        QString direct_dns = BOX_UNDERLYING_DNS;
        if (dataStore->block_ads_malware) {
            remote_dns = "94.140.14.14";
            direct_dns = "94.140.14.14";
        }

        auto config = ReadFileText(configFn)
                          .replace("//%TOR_OUTBOUND%", tor_outbound)
                          ...
                          .replace("%REMOTE_DNS_ADDRESS%", remote_dns)
                          .replace("%DNS_ADDRESS%", direct_dns)
