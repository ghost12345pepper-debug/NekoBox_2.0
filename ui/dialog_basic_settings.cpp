...
DialogBasicSettings::DialogBasicSettings(QWidget *parent)
    ...
    D_LOAD_INT_ENABLE(auto_ping_interval, auto_ping_interval_enable)

    // Core

    ui->groupBox_core->setTitle(software_core_name);
    
    // Add Tor Mode checkboxes
    auto chkTorSys = new QCheckBox(tr("Включить Tor режим на системный прокси (System Proxy)"), this);
    auto chkTorTun = new QCheckBox(tr("Включить Tor режим на режим TUN"), this);
    chkTorSys->setChecked(NekoGui::dataStore->tor_system_proxy);
    chkTorTun->setChecked(NekoGui::dataStore->tor_tun_mode);
    connect(chkTorSys, &QCheckBox::toggled, this, [=](bool c){ NekoGui::dataStore->tor_system_proxy = c; });
    connect(chkTorTun, &QCheckBox::toggled, this, [=](bool c){ NekoGui::dataStore->tor_tun_mode = c; });
    ui->groupBox_core->layout()->addWidget(chkTorSys);
    ui->groupBox_core->layout()->addWidget(chkTorTun);
    //
...
void DialogBasicSettings::accept() {
    ...
    if (ui->auto_ping_interval_enable->isChecked()) {
        TM_auto_ping_Reset_Minute(ui->auto_ping_interval->text().toInt());
    } else {
        TM_auto_ping_Reset_Minute(0);
    }
    D_SAVE_INT_ENABLE(auto_ping_interval, auto_ping_interval_enable)
