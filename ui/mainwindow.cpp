...
#include "ui/dialog_ping_settings.h"
...
#include <QDir>
#include <QFileInfo>
#include "widget/TrafficGraphWidget.h"
...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow) {
    ...
    ui->horizontalLayout_2->insertWidget(2, trafficGraph);
    ...
    ui->horizontalLayout_2->insertWidget(ui->horizontalLayout_2->indexOf(ui->toolButton_url_test), toolButton_testPing);
    autoPingTimer = nullptr;
    setupAutoPingTimer();
    
    toolButton_testPing->setContextMenuPolicy(Qt::CustomContextMenu);
    connect(toolButton_testPing, &QWidget::customContextMenuRequested, this, [=](const QPoint &) {
        DialogPingSettings dialog(this);
        dialog.exec();
        setupAutoPingTimer();
    });
    connect(toolButton_testPing, &QToolButton::clicked, this, [=]() {
        if (QApplication::keyboardModifiers() & Qt::ShiftModifier) {
            DialogPingSettings dialog(this);
            dialog.exec();
            setupAutoPingTimer();
        } else {
            // run test on selected based on settings
            int testType = NekoGui::dataStore->auto_ping_type_selected;
            if (testType == 0) {
                speedtest_current_group(1, false); // URL Test selected
            } else {
                speedtest_current_group(0, false); // TCP Ping selected (assuming 0 is TCP)
            }
        }
    });

    // Setup Ad Blocker Checkbox
    auto checkBox_adBlock = new QCheckBox(tr("Block Ads && Malware"), this);
    checkBox_adBlock->setChecked(NekoGui::dataStore->block_ads_malware);
    ...
    TM_auto_ping = new QTimer;
    TM_auto_ping_Reset_Minute = [&](int m) {
        TM_auto_ping->stop();
        if (m > 0) TM_auto_ping->start(m * 60 * 1000);
    };
    connect(TM_auto_ping, &QTimer::timeout, this, [&] { speedtest_current_group(0, true); });
    TM_auto_ping_Reset_Minute(NekoGui::dataStore->auto_ping_interval);
...
void MainWindow::updateTrafficGraph(quint64 up, quint64 down) {
    if (trafficGraph) {
        trafficGraph->addData(up, down);
    }
}
...
void MainWindow::setupAutoPingTimer() {
    if (!autoPingTimer) {
...
void MainWindow::onAutoPingTriggered() {
    // Determine what to ping based on settings
    int testType = NekoGui::dataStore->auto_ping_type_all;
    if (NekoGui::dataStore->auto_ping_check_all) {
        if (testType == 0) {
            speedtest_current_group(1, false); // URL Test for all
        } else {
            speedtest_current_group(0, false); // TCP Ping for all
        }
    } else if (NekoGui::dataStore->auto_ping_include_multi && !NekoGui::dataStore->auto_ping_selected_servers.isEmpty()) {
        // Technically we need a custom function to test specifically selected proxies.
        // For now, this invokes the standard test on the active proxy as a placeholder.
        int selTestType = NekoGui::dataStore->auto_ping_type_selected;
        if (selTestType == 0) {
            speedtest_current_group(1, false);
        } else {
            speedtest_current_group(0, false);
        }
    }
}
